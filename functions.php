...
function draw_header($pageTitle, $lastmessage='')
{
    ...
    $GLOBALS['smarty']->assign('acceptRevisions', $GLOBALS['CONFIG']['acceptRevisions']);
...
function draw_footer()
{
    ...
    include(ABSPATH . '/templates/' . $GLOBALS['CONFIG']['theme'] . '/footer.tpl');
    //display_smarty_template('footer.tpl');

    /**
     * Stops the performace timer and displays it if in debug mode
     * @var [type]
     */
    $performanceTimer = microtime(true) - $performanceTimer;
    if ($GLOBALS['CONFIG']['debug'] == 'True') {
        echo "\n\n<!--\n\n\tLoad time: $performanceTimer\n\n-->";
    }
}
