...
$downloads = $file_data_obj->getDownloads();
...
if ($user_permission_obj->getAuthority($_REQUEST['id'], $file_data_obj) >= $user_permission_obj->READ_RIGHT) {
    ...
    $suffix = strtolower((substr($file_data_obj->name,((strrpos($file_data_obj->name,".")+1)))));
    $mimetype = File::mime_by_ext($suffix);
    $view_link = 'view_file.php?submit=view&id=' . urlencode($file_data_obj->id).'&mimetype='.urlencode("$mimetype");
