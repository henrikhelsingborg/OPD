...
$query = "INSERT INTO {$dbprefix}department VALUES (NULL, 'Public')";
$stmt = $pdo->prepare($query);
$stmt->execute();
...
// Create public user
$query = "INSERT INTO {$dbprefix}user VALUES (NULL,'public',md5('publicpassword'),'2','','publicuser@helsingborg.se','User','Public','', 0, 0)";
$stmt = $pdo->prepare($query);
$stmt->execute();
...
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'theme', 'hbg', 'Which theme to use?', '');",
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'language', 'swedish', 'Set the default language (english, spanish, turkish, etc.). Local users may override this setting. Check include/language folder for languages available', 'alpha|req');",
